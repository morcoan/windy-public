...
pub struct Project {
    ...
    /// Memory layout / VAâ†”offset translations.
    ...
    /// Rename lineage for symbols (old â†’ new), agent-queryable.
...
impl Project {
    ...
    /// Optimized SSA form of a function â€” copy/constant propagation, trivial-phi
    ...
    /// its optimized SSA. Read-only â€” persistence is the caller's job through
    ...
    /// Native (non-LLM) pseudo-C decompilation â€” returns artifact text only.
    ///
    ...
    pub fn function_dataflow_json(
        ...
        // Build use map: def â†’ list of consumer var labels (or return:va).
        ...
        for block in &ssa.blocks {
            ...
            // Already covered when we see them as uses of ops that define from nothing â€”
    ...
    pub fn call_sites_with_args(&self, va: u64) -> Option<serde_json::Value> {
        ...
        // Map def var â†’ defining op for source classification.
            ...
            for op in &block.ops {
                ...
                for (rank, (base, reg)) in arg_regs.iter().enumerate().take(max_args) {
                    ...
                    let (source, ty_str) = if let Some(var) = reaching {
                        ...
                        // No reaching def found â€” still emit Win32 expected type.
                        let ty_str = sig
                            .as_ref()
                            .and_then(|s| s.params.get(rank))
                            .map(|(_, t)| self.types.render(t));
    ...
    /// Apply a weak keyâ†’name map against the focused function.
    ///
    /// Supported keys:
    /// - `__function__` â€” rename the focused function
    /// - `local:-0x10` / `local:-16` â€” rename stack local at offset
    /// - `arg:0` / `arg:1` â€” rename signature parameter by index
    /// - anything else â€” stored as a function-scope comment for operator review
...
fn pcode_op_name(op: &rsleigh_api::PcodeOp) -> String {
    let s = format!("{op:?}");
    // "IntAdd { out: ..., left: ..., right: ... }" â†’ "IntAdd"
    s.split_whitespace()
        .next()
        .unwrap_or("Op")
        .trim_end_matches('{')
...
fn classify_arg_source(
    ...
    if let Some(op) = def_op {
        ...
        // LEA / IntAdd of RIP-relative address â†’ often a string pointer.
        if matches!(
            &op.kind,
...
/// If `source` embeds a VA (constant:/global:â€¦@va) that points at a string, return it.
...
fn resolve_switch_infos(
    ...
    // Map target entry VA â†’ SSA block index.
...
mod tests {
    ...
    fn phase6_call_constraints_seeded_from_win32_db() {
        ...
        // 1) IAT slot â†’ SigDB signature (the call_constraints seed source).
        let iat = project
            .symbols
            .iter()
            .find(|(_, s)| {
        ...
        // If no SSA-level constraint was recovered, the IATâ†’SigDB path above
    ...
    /// Non-Grand PE corpus: pure_no_fallback V2 share â‰¥ 99% (not Grand proxy).
    #[test]
    fn external_corpus_pure_v2_share() {
        let root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let pes = [
            root.join("tests/fixtures/sample.exe"),
            root.join("tests/fixtures/complex.exe"),
        ];
        let pure = crate::decompiler::v2::DecompileOptions::pure_no_fallback();
    ...
    fn native_sample_main_emits_recovered_win64_call_arguments() {
        ...
        // Count top-level commas only â€” nested `*(arg_20)` must not truncate the
    ...
    /// Count comma-separated arguments inside the first call `(â€¦)` on a line,
    ...
    fn phase7_points_to_distinct_globals_on_sample() {
        ...
        // Soft: sample may resolve 0â€“N globals depending on code shape.
