...
mod tests {
    ...
    /// Dense classify if-ladder should fold to `switch` for soft/control gold.
    #[test]
    fn c02_classify_p1_pure_v2_surfaces_switch() {
        use crate::project::Project;
        let pe = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        let dir = std::env::temp_dir().join("windy-ratchet-c02-switch");
        let _ = std::fs::create_dir_all(&dir);
        let entry = 0x1400_01000u64;
        let project =
            Project::open_with_data_dir_and_entry_hints(&pe, &dir, &[entry]).expect("open");
        ...
        assert!(
            ...
            "c02 classify must surface switch from eq ladder, got:\n{}",
            art.text
        );
    }
