//! Native pseudo-C emission over optimized SSA â€” Phase 5.1 (full DREAM).
//!
...
pub struct NameCtx<'a> {
    ...
    /// VA â†’ annotated symbol name (from [`crate::ir::annotate::build_global_names`]).
    pub global_names: HashMap<u64, String>,
    /// Instruction VA â†’ data-section global VA (from `resolve_global_va`).
...
///
/// Pure V2 path: raw region emit â†’ **CfgOnly only**.
///
...
/// CfgOnly / LegacySemantic text surgery â€” that is presentation polish.
///
...
pub fn structure_emit_core(
    ...
    if !ssa.blocks.is_empty() {
        // Prefer the SSA block whose entry_va matches the function entry â€”
    ...
    for i in 0..ssa.blocks.len() as u32 {
        ...
        // covered reducible control. Skip pure-control residual blocks (1.txt Â§1).
    ...
    out.push_str("}\n");
    // Raw region tree only â€” CfgOnly / LegacySemantic applied by pure/legacy entry points.
    out
}
...
/// LegacySemantic only â€” not on the pure path.
...
pub(crate) fn polish_hoist_rich_xor_return(src: &str) -> String {
    ...
    // Pattern: if ((â€¦== 0â€¦)) { â€¦ } else { return 0; } then later rich return.
    let mut i = 0usize;
...
/// When a function does `FUN_(slotA, 1); FUN_(slotB, 2); â€¦ FUN_(slotB); FUN_(slotA);`
    ...
    while i < lines.len() {
        ...
        if is_if_open && i + 2 < lines.len() {
            ...
            if mid.starts_with("return") && mid.ends_with(';') && close == "}" {
                // Skip if next is `else` â€” keep structured if/else.
        ...
        // `else {\n return x;\n }` â†’ `else return x;` (trailing early-exit arm).
...
/// When â‰¥2 consecutive `FUN_â€¦(stack/fp)` calls appear after a loop (reverse
...
pub(crate) fn polish_paired_cleanup_destroys(src: &str) -> String {
    ...
    // Forward scan: after a loop, collect runs of â‰¥2 fp/stack FUN_ calls.
    let mut i = 0usize;
...
/// Replace returns that are algebraically zero (`x + (1*0)*1`, `x ^ x`, â€¦) with `return 0`.
...
/// Collapse MSVC zero dual-flag soup: `(x == 0x0),(0x0 != (x < 0x0))` â†’ `(x == 0x0)`.
...
pub(crate) fn polish_e_pointer_returns(src: &str) -> String {
    ...
    while i < lines.len() {
        ...
        // One-line guards: COM null â†’ E_POINTER. Fire for VARIANT tags or QI shape.
        ...
        // Bare `return 0x80004003;` â†’ assign + return for structure align.
...
/// `if (!(p == 0)) { BODY } else { return EP; }` â†’ `if (p == 0) return EP;\n BODY`
    ...
    while i < lines.len() {
        ...
        if k < lines.len() {
                ...
                && et.contains("8000")
            {
                ...
                || et == "else {"
            {
                // Scan else block for HRESULT return (may be preceded by `hr = â€¦`).
                let mut m = k + 1;
...
pub(crate) fn strip_flag_helper_noise(src: &str) -> String {
    // Strip `/*(IntSBorrow â€¦)*/`, `/*(IntSLess â€¦)*/`, `/*(Boolâ€¦)*/` style comments.
    let mut out = String::with_capacity(src.len());
    let b = src.as_bytes();
    let mut i = 0;
    while i < b.len() {
...
/// Fold nested `if ((scrut - C) == 0) { body } else { if ((scrut - D) == 0) â€¦`
/// ladders into `switch (scrut) { case C: body; â€¦ }` when â‰¥2 arms share the
...
/// span for the chosen scrutinee is rewritten â€” outer guards stay intact.
pub(crate) fn fold_eq_ladder_to_switch(src: &str) -> String {
    ...
    let Some((scrut, case_ks)) = by_scrut
        ...
        .filter_map(|(s, v)| {
            ...
            // Need â‰¥2 distinct constants; pure {0} is not a user tag dispatch.
            if small_n >= 2 && !magic {
                Some((s, ks))
            } else {
                None
    ...
    let Some((cases, end)) = extract_eq_ladder_span(src, start, &scrut, &case_ks) else {
        // Fallback: structural fold without body capture (empty arms) â€” only
...
///
/// Nested MSVC shape `else { if (scrut-K) {â€¦} else {â€¦} }` is peeled iteratively
/// so a final single arm is still collected (caller requires â‰¥2 labeled arms).
fn extract_eq_ladder_span(
    ...
    let mut default_body = String::new();
    // Work buffer for nested `else { if â€¦ }` bodies we re-scan.
    ...
    for _ in 0..16 {
        ...
        if ae_compact.starts_with("if(") {
            // else if â€” continue peel on same work buffer
            cursor = work_start;
            continue;
        }
        // else { â€¦ }
    ...
    // string â€” recover top-level end by scanning brace balance from original start.
...
fn find_ladder_end(src: &str, start: usize) -> Option<usize> {
    ...
    // Walk if { } else { } else if { } â€¦ consuming brace groups and else keywords.
    let bytes = src.as_bytes();
    loop {
        ...
        if i + 4 <= bytes.len() && &src[i..i + 4] == "else" {
            ...
            // else if â†’ find next `{` of that if
...
fn collect_eq_ladder_arms(src: &str) -> Vec<(String, i64, String)> {
    ...
    for line in src.lines() {
        ...
        // Form B: SCRUT==0xK)  (direct equality ladder) â€” skip ==0 only (null checks).
...
/// Recover `L: body; if (c) goto L;` â†’ `while (c) { body; }` when the body
...
/// cookie global or `0x14001â€¦` image address.
pub(crate) fn strip_security_cookie_gotos(src: &str) -> String {
    ...
    // when the preceding context mentions the GS cookie global (g_14â€¦).
    ...
    for line in &lines {
        ...
        if let Some(lab) = parse_goto_loose(t) {
            // Only rewrite gotos into pure fail/return leaves â€” never ordinary merges.
            let is_fail = fail_leaves.contains(&lab)
                || (cookie_context && label_is_trivial_return(&lines, &lab));
            // Orphaned goto (label never emitted) â†’ fail return. Cookie context
            ...
            if is_fail || orphan {
                ...
                // Present as a clean return â€” do not leave "gs-cookie" markers in
...
fn is_bare_fail_return_line(l: &str) -> bool {
    ...
    // Only empty / zero / self-xor returns count as fail epilogue â€” never
...
/// so residual goto mass drops without changing path effects (1.txt Â§1 budget).
pub(crate) fn inline_leaf_goto_targets(src: &str) -> String {
    ...
    for line in &lines {
        ...
        // Drop labels we fully inlined if no remaining gotos to them â€” second pass.
        out.push(line.clone());
    }
...
/// Fold `if (â€¦) { goto L; } â€¦ L: return â€¦` and remove comments-only goto noise.
...
pub(crate) fn fold_goto_return_and_trivial_rejoins(src: &str) -> String {
    let lines: Vec<String> = src.lines().map(|s| s.to_string()).collect();
    // Build label â†’ line index for simple L_â€¦: labels.
...
struct EmitCtx<'a> {
    ...
    ve: u32,
    indent: usize,
    /// Nested While/DoWhile frames (innermost last) â€” stage 8 path-word cleanup.
...
fn emit_region(
    ...
    while let Some(b) = current {
        ...
        if emitted.contains(&b) {
            // Already emitted: loop back-edge â†’ continue/break; else unstructured goto.
        ...
        // Stage 8: never label loop headers â€” back edges become continue/break.
        ...
        // Short-circuit &&/|| (S5) â€” best-effort before region match.
        ...
        match ctx.regions.get(&b).cloned() {
            ...
            Some(Region::IfThenFallthrough {
                ...
                ctx.indent -= 1;
                out.push_str(&format!("{}}}\n", ind(ctx.indent)));
                // Sequential continuation (no else brace) â€” early-return form.
...
fn try_emit_goto_alternative(
    ...
    // Jump into a return block â†’ emit return in place (shared epilogue).
    if block
        .ops
        .iter()
    ...
    // Unemitted structured region root â€” emit it rather than goto.
    ...
    match succs.len() {
        ...
        1 => {
            ...
            if let Some(kw) = loop_edge_kw(ctx, s) {
                // `continue` at the end of the body is implicit â€” omit.
                if kw == "continue" {
                    return None;
                }
                out.push_str(&format!("{}{};\n", ind(ctx.indent), kw));
...
fn recover_pred_rax_return(
    ...
    for b in &ssa.blocks {
        ...
        for op in &b.ops {
            ...
            // Reject pure self-xor / cookie epilogue forms (`x ^ x`, `fp ^ â€¦`).
...
fn recover_cmp_sete_return(
    ...
    for op in &block.ops {
        ...
        // cmp X, imm â†’ IntSub(tmp, X, imm) then IntEq(ZF, tmp, 0)
...
fn emit_return(
    ...
    // cmp/sete boolean returns (SEH filters, equality kernels) â€” before arithmetic
    ...
    // HRESULT facility constants (0x8000xxxx) only â€” avoid stealing ordinary
    ...
    if let Some(ssa) = ssa {
        ...
        // Zero-xor / +1 noise may still appear as IntAdd/IntXor â€” do not require
        ...
        if thin_fail_arm {
            ...
            for b in &ssa.blocks {
                for op in &b.ops {
                    ...
                    // 64-bit immediate (`0xffffffff80004003`) â€” match on low 32 bits.
    ...
    // compound term (e.g. `a + b`) â€” never frame-pointer address math.
    ...
    // Run before sink fallback so cookie `fp ^ â€¦` noise cannot win.
    if let Some(ssa) = ssa
    ...
    // Stack-local reloads only win when RAX is dual-flag soup or missing â€”
    // otherwise SEH filters (cmp/sete â†’ RAX) lose to epilogue loads.
    let mut rax_rv: Option<String> = None;
    let mut stack_local_rv: Option<String> = None;
    let mut term_va = 0u64;
    // Track last IntEq / comparison that can feed sete â†’ RAX boolean returns.
    ...
    for op in &block.ops {
        if let Some(def) = &op.def {
                ...
                || matches!(&op.kind, SsaOpKind::Pcode(PcodeOp::Load { .. }))
            {
                ...
                // Stack local / simple load â€” not flag soup, not frame math.
...
/// (`if (cond) goto exit; fall body` â†’ `while (!cond)`).
fn while_cond(
    block: &SsaBlock,
    body_entry: u32,
    inline_exprs: &HashMap<SsaVar, String>,
...
fn do_while_cond_str(cond_block: &SsaBlock, body_entry: u32, cond: &str) -> String {
    // Back edge on fallthrough means we loop when cond is false â†’ invert.
...
// â”€â”€â”€ Expression folding (unchanged from MVP) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
fn build_inline_exprs(
    ...
    // m_d â‰¥ 2 (cheap casts always; arithmetic up to a small use bound; loads
        ...
        for (op, _) in &flat.ops {
            if let Some(def) = &op.def {
                ...
                let fold = if crate::decompiler::normalize::is_cheap_contractible(op) {
                    ...
                    // Lemma 9: multi-use value-class reconstruction â€” fold shared
...
/// Bare identifier (no operators) â€” a pure rename after SI.
...
fn emit_block_statements(
    ...
    for (operation_index, op) in block.ops.iter().enumerate() {
        ...
        // Stage 1: dominated parameter-home stores are pure `y = p` echoes â€”
        ...
        // Stage 6: dead pure defs (no live uses) â€” SI residual DCE at print time.
        if let Some(def) = &op.def {
            let uses = use_count.get(def).copied().unwrap_or(0);
        ...
        // Stage 6 SI: already contracted into uses â€” do not materialize.
        if let Some(def) = &op.def
        ...
        match &op.kind {
            SsaOpKind::Pcode(PcodeOp::Store { ptr, val, .. }) => {
                ...
                // Prefer named stack slot assignment over `*((off + rsp)) = â€¦`.
                if let Some(def) = &op.def
...
fn name_of(sv: &SsaVar, names: &NameCtx<'_>, op_va: Option<u64>) -> String {
    match &sv.location {
        Location::Register { base_offset } => {
            // Param registers at version 1 â†’ signature param names.
            if sv.version == 1
...
fn render_op_expr(
    ...
    match &op.kind {
        ...
        // 1.txt workstream 2: signed relational predicates â€” print as source-like
        // relations, never as `/*(IntSLess â€¦)*/ left,right` flag soup.
...
/// Stage 7: null-sentinel only for proven byte/char probes â€” never for general
...
fn render_call_target(
    ...
    if let Some(rest) = rendered.strip_prefix("sub_") {
        ...
        // Direct call encoded as ram pointer â€” emit FUN_ when it matches a known entry.
...
// â”€â”€â”€ S6: goto minimization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub(crate) fn minimize_gotos(src: &str) -> String {
    ...
    // Lemma 10: recurrence normalization â€” while(1){ if(!(B)) break; ... }
            ...
            if j < lines.len() {
                ...
                if let Some(cond) = parse_if_not_break(nxt) {
                    ...
                    if i < lines.len() && lines[i].trim() == "}" {
                        // might be if's closing â€” check if break was single-line
...
mod tests {
    ...
    fn diamond_emits_if_else_without_goto() {
        // 0 cbranch fall=else(2) taken=then(1); both â†’ join(3 return)
    ...
    fn switch_emits_cases_and_break() {
        // Block 0: BranchInd â†’ 1, 2; both â†’ merge 3.
        let ind = SsaOp {
            va: 0x1000,
    ...
    fn short_circuit_and_or_nested_if() {
        // B1 fallâ†’B2, takenâ†’false(3); B2 fallâ†’true(4), takenâ†’false(3).
    ...
    fn minimize_removes_fallthrough_goto() {
        ...
        // Label also unused now â†’ stripped.
        assert!(
            !out.contains("L_0x10:"),
    ...
    fn sample_exe_native_uses_reg_names_not_r08() {
        ...
        for f in project.functions().iter() {
            ...
            // Prefer human names: rcx / rdx / param names â€” never r08_1.
    ...
    fn stage6_identical_pure_ops_share_surface() {
        // Two independent zexts of the same source â€” only one materialization
...
"#;
    ...
    fn pure_v2_never_runs_semantic_polish() {
        ...
        // Pure = CfgOnly(raw) exactly â€” no polish_*, no emit_finalize.
        assert_eq!(
            pure,
            apply_cfg_only(&raw),
...
"#;
        ...
        assert!(
            out.contains('^') || out.contains("45d9"),
            "later xor return must remain visible, got:\n{out}"
        );
    }
}
