...
//! P-code op semantics (arithmeticâ†’Int, comparisonsâ†’Bool, float opsâ†’Float/
//! Double, Copy/phi unify). A *backward* pass derives operand constraints
//! (`Store` ptr â†’ `Ptr(val)`, `Load` ptr â†’ `Ptr(out)`, arithmetic operands â†’
//! `Int(size)`, Copy input â†’ def type), which types **live-in parameters**
//! (registers/stack-slots with no defining op).
//!
//! Recovered types project onto:
//! * **stack locals/args** â€” `Location::StackSlot` defs â†’ `StackFrame`;
//! * **parameters** â€” fastcall GPR live-ins (RCX/RDX/R8/R9), XMM float live-ins,
//!   and cdecl positive-disp stack-slot live-ins;
//! * **return type** â€” meet of every `Return` register use;
//! * **global candidates** â€” `RawRam` Store/Load with a typed value, carrying
...
/// x64 fastcall parameter registers (SLEIGH container bases) â†’ ABI rank.
...
pub struct TypeRecoveryReport {
    ...
    /// older tests that construct reports by hand â€” default empty.
...
/// Recover types for the function at `va` over its optimized SSA. Read-only â€”
...
fn seed_call_constraints(
    ...
    for block in &ssa.blocks {
        // base_offset â†’ current version at this program point
        ...
        for op in &block.ops {
            ...
            for (i, arg_ty) in constraint.arg_types.iter().enumerate() {
                ...
                // â€” re-walk uses is fine; fall back to version 1.
...
/// is a `(SsaVar, TyGuess)` to unify into the lattice â€” this types live-in
...
fn operand_constraints(op: &SsaOp, types: &HashMap<SsaVar, TyGuess>) -> Vec<(SsaVar, TyGuess)> {
    ...
    match &op.kind {
        ...
        // Signed binary ops: both operands are Int(width) â€” strong signed evidence.
        SsaOpKind::Pcode(
        ...
        // Unsigned binary ops: both operands are Uint(width) â€” strong unsigned evidence.
        SsaOpKind::Pcode(
        ...
        // no signedness constraint â€” let meet resolve from stronger ops.
...
mod tests {
    ...
    fn mixed_signed_unsigned_meets_to_uint() {
        // Same live-in used in IntSLess (Int) and IntDiv (Uint) â†’ meet â†’ Uint.
        let cmp = SsaOp {
            va: 0x1000,
            kind: SsaOpKind::Pcode(PcodeOp::IntSLess {
                out: Varnode::register(0x00, 1),
    ...
    fn call_constraint_seeds_arg_register() {
        // Call at 0x1000 with constraint RCX â†’ Int(32). Live-in RCX version 1
