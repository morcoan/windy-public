//! P-code lifter — Phase 0 of WindyDec.
//!
...
//! compile — Windy links a single self-contained binary that owns its lifter.
//!
//! P-code vocabulary mirrors Ghidra's [`pcoderef`] exactly (op names such as
//! `IntAdd`, `Store`, `Load`, `Copy`, `CBranch`). Downstream phases — SSA
//! construction (Phase 2), TRex type recovery (Phase 4), DREAM structuring
//! (Phase 5) — all consume this standard IR.
//!
...
/// so the default libtest worker stack (2 MiB) — and the default tokio worker
/// stack — overflows even for trivial instructions. We give the decode its own
...
/// Lift every instruction in `func` to P-code on a dedicated ≥128 MiB stack.
///
...
pub fn lift_function_blocking(
    ...
    // Gather the (va, bytes) pairs on the caller's stack — cheap, since each
...
mod tests {
    ...
    /// `mov [rsp+0x10], edx` (Intel) — the Phase 0 verification gate instruction.
    ///
    ...
    /// `add eax, edx` — the heart of `add(int a, int b)`.
    ...
    fn test_pcode_lift_add() {
        with_big_stack(|| {
            ...
            // The Store's pointer must be exactly the IntAdd's output varnode — i.e.
    ...
    fn test_pcode_lift_control_flow() {
        with_big_stack(|| {
            let mut lifter = SleighLifter::new();
            // `cmp ecx, edx` (39 d1) then `jg` (7f xx) — used by max3 decision tree.
