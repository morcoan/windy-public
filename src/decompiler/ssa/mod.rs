//! Function-level SSA over per-instruction P-code — Phase 2 of WindyDec.
//!
...
/// `<frame_ptr_reg> ± const` pattern become [`Location::StackSlot`]; everything
...
pub enum Location {
    ...
    /// Unresolved RAM — tracked as one opaque memory token (alias analysis deferred).
    RawRam,
}
...
/// [`lift_function_blocking`] (which already runs the decoder on a ≥128 MiB
...
mod tests {
    ...
    /// A function containing an `IntAdd` — the "add" / arithmetic function.
    ...
    fn test_ssa_add_function_unrenamed_uses_zero() {
        ...
        // the uninitialized `0` sentinel — i.e. no un-renamed reads).
