...
| Key | Required | Notes |
|---|---|---|
| `summary` | yes | Structural card (name, va, size, blocks, …) |
...
```json
{ "kind": "call|data|insn|symbol|stack|summary", "va": "0x…", "note": "optional" }
```

- `kind=call` — call-site or callee edge  
- `kind=data` — string / global data VA  
- `kind=insn` — instruction defining a constant or points-to  
- `kind=symbol` — symbol table entry  
- `kind=stack` — stack offset (use `note` for offset)  
- `kind=summary` — derived aggregate with no single VA  

## Caps

- Default `max_items` = 32, hard max 64 per list section  
- Optional agent_text: default max 64 instructions  
- Implementers should keep typical cards well under ~2–4k tokens  
...
- memory card missing but function has ≥1 import API  
