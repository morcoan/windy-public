...
| `memory_purpose_set` | — | Function memory card has non-empty purpose |
...
| edge | Meaning |
|---|---|
| `calls` | code → code/import |
| `xref` | general cross-reference |
| `imports` | module → external API |
| `exports` | module → export |
| `data_ref` | code → data/string |
| `part_of` | local/param → function |
| `same_as` | cross-module identity (workspace) |
| `documented_in` | entity → comment/memory |
...
Each `verify_claims` batch may append JSONL records under the project’s claim journal for offline calibration (verdict, kind, checker version).
